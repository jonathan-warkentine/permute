var mostSongsInPlaylist = function(arr) {
    ...
    const playlists = permute(arr).map(permutation => {
      return {
        'numSongs': permutation.length,
        'duration': permutation.reduce((acc, currentVal) => acc + currentVal)
      }
    });
    
    const playlistsUnderHour = playlists.filter(playlist => {
      return playlist.duration < 60;
    });

    return playlistsUnderHour.reduce((acc, playlist) => playlist.duration > acc? playlist.duration: acc, 0) || 0;
